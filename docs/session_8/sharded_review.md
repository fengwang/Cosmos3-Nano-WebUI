...
|---|---|---|---|---|
| R-F1 | **High** | `/workspace/github.repo/vllm-omni` local absolute path in `evidence_map.md:21` (INV-1 / §6). The committed scanner's `PRIVATE_PATH_PATTERNS` has no `/workspace/` class, so #12 was clean by construction (contract adversarial case "scan scoped too narrowly"). | security | **FIXED in-radius** — replaced with public phrasing; corrected #15 scan re-verified the in-radius surface clean. |
...
| R-F3 | **Med** | 24 `/workspace/github.repo/…` local paths in **out-of-radius** historical docs (`docs/session_2/**` = 22 `vllm-omni`; `session_3/plan.md`+`session_4/plan.md` = 2 repo self-path). Tracked → ship at publish. | security | **Owner decision** — `failure_arbiter.md` FA-2 (SPEC_GAP / blast-radius). Cannot edit without amendment. GO condition. |
...
| R-F8 | **Nit** | Weight/media extension set inconsistent (#13 has `.webm`, #14/scanner/checklist differ); scanner lacks `.webm`. Moot (0 `.webm` files exist). | tests | **Noted** — `failure_arbiter.md` FA-3 (scanner recommendation for owner); deterministic_checks Notes updated. |
...
## Owner decision required (not fixable in the docs-only blast radius)

- **FA-2 (R-F3):** approve a blast-radius amendment to scrub `/workspace/…` from
  `docs/session_{2,3,4}/**` (mechanical; re-verify after), **or** accept as non-sensitive /
  non-blocking for beta. Recommendation: scrub for a clean public release.
- **FA-3 (R-F8):** owner to add a `/workspace/` pattern class (and `.webm`) to
  `tests/test_private_ref_scan.py` so the automated gate catches this class going forward
  (product code; eval seed `mig_s8_scanner_abspath_blindspot`).

## Verdict

No **Critical**. All in-radius High/Med findings **fixed**; the two out-of-radius items are
classified and routed to the owner. The GO recommendation stands, now **conditional on the
FA-2 owner decision** in addition to the standing GPU + at-publish conditions.
