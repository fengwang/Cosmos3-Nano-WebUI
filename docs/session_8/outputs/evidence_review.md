...
| The curated import is complete (no hollow test pass) | `evidence_map.md`: 296 files; `compileall api`=0; torch-free `import app.main`=0; `pytest -m "not gpu"` = **485 passed** (`deterministic_checks.md` #4); excluded-module import grep clean | verified-now |
| CPU-only CI is configured and every step passes locally | `.github/workflows/ci.yml` (python+webui, `permissions: contents: read`, no secrets); `deterministic_checks.md` #2–#9 (ruff 0, pytest 485, vitest 209, build/lint/typecheck green); two schema-sync gates proven to fail on injected drift (S5) | verified-now |
