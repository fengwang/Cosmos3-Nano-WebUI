...
class Cosmos3ForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    hf_to_vllm_mapper = WeightsMapper(
        ...
        orig_to_new_substr={
            ...
            ".norm_q.": ".q_norm.",
            ".norm_k.": ".k_norm.",
            # AM-S2: modelopt blockwise-FP8 scale sidecars. Map the per-block
            # `_scale` into vLLM's `weight_scale` param (consumed by the W8A16
            # linear method) and drop the informational `_amax` twin. Without
            # this the AutoWeightsLoader hits an unmapped key (KeyError) or the
            # scale is silently dropped.
            ".weight_quantizer._scale": ".weight_scale",
            ".weight_quantizer._amax": None,
        },
        orig_to_new_prefix={
            "proj_in.": None,
            "proj_out.": None,
